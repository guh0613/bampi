...
async def resolve_chromium(
    *,
    ...
    system = find_chromium(executable_path)
    if system is not None:
        return system
    resolved_cache = Path(cache_dir).expanduser() if cache_dir else default_cache_dir()
    cached = find_cached_chrome(resolved_cache)
    if cached is not None:
        return str(cached)
    if auto_install:
        return str(await ensure_chrome_for_testing(resolved_cache, timeout=install_timeout))
    raise BrowserLaunchError(
        "No Chromium browser was found and automatic Chrome for Testing installation is disabled. "
        "Install Chrome/Chromium or configure a browser executable path."
    )
...
async def launch_chromium_process(
    args: Sequence[str],
    *,
    executable: str,
    profile_dir: Path,
    ...
    if websocket_url is None:
        ...
        with suppress(Exception):
            await process.wait()
        raise BrowserLaunchError(f"Chromium did not expose DevTools within {launch_timeout:g}s.")

    try:
        client = await CdpClient.connect(websocket_url, timeout=launch_timeout)
