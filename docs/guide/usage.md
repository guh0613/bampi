...
消息通过触发、限流与并发检查并开始新一轮处理后，Bot 会给这条消息临时贴上 QQ 原生的「仔细分析」表情，完成回复或异常/中止收尾时自动撤销。这个状态回应不会写入对话上下文；追加到已有轮次的消息也不会重复显示。
...
- **Live text stream（兼容配置名）**：当 Agent 接下来要调用工具时，先发送该轮已经写完的中间文字；中间文字和最终回复都会正确渲染代码、公式和表格，最终回复仍在完整生成后统一发送。

开关细节见 [配置参考](configuration.md) 中「会话与回复」一组。上下文自动压缩开始时，也可能收到一句压缩提示（可关）。
